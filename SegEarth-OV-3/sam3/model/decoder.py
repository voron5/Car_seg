...
class TransformerDecoder(nn.Module):
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if hasattr(self, "compilable_stored_size") and self.compilable_stored_size == (H, W):
            # используем кэшированные координаты дл€ известных размеров
            coords_h, coords_w = self.compilable_cord_cache
        else:
            # cache miss Ч создаЄм координаты заново и кэшируем в словаре
            if feat_size not in self.coord_cache:
                self.coord_cache[feat_size] = self._get_coords(H, W, reference_boxes.device)
            coords_h, coords_w = self.coord_cache[feat_size]

            # проверка размеров координат
        ...
        # проверка размеров без обращени€ к torch.compiler
